...
pipeline {
  ...
  stages {
    stage('Checkout') {
      steps {
        git branch: 'main', url: 'https://github.com/sivas-git/springboot-build-pipeline.git'
      }
    }
   ...
   stage('Stage VII: Scan Image ') {
      steps { 
        ...
        sh "trivy image --scanners vuln --offline-scan siva3r/spring:latest > trivyresults.txt"
        }
    }
   ...
   stage('Stage VIII: Smoke Test ') {
      steps { 
        ...
        sh "docker run -d --name smokerun -p 8000:8000 siva3r/spring"
